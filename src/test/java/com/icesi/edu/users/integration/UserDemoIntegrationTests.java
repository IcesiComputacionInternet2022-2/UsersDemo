...
public class UserDemoIntegrationTests {
    ...
    private void init(){
        ...
        userDTO = objectMapper.readValue(parseResourceToString("createUser.json"), UserDTO.class);
    }
    ...
    public void createUserSuccessfully(){
        ...
        assertThat(userResult, hasProperty("email", is("juan.prada@icesi.edu.co")));
        assertThat(userResult, hasProperty("phoneNumber", is("+573014743258")));
        assertThat(userResult, hasProperty("firstName", is("Juan")));
        assertThat(userResult, hasProperty("lastName", is("Prada")));

    }

    @SneakyThrows
    public void createUserInvalidName(){
        userDTO.setFirstName("Pepito$$$");
        String body = objectMapper.writeValueAsString(userDTO);
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/users")
                .contentType(MediaType.APPLICATION_JSON)
